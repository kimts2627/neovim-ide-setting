...
function findGraphQLTags(text, ext, uri, logger) {
    ...
    try {
        parsedAST = (0, parser_1.parse)(text, PARSER_OPTIONS);
    }
    catch (error) {
    ...
    const parseTemplateLiteral = (node) => {
        ...
        if (loc) {
            ...
            const range = new graphql_language_service_1.Range(new graphql_language_service_1.Position(loc.start.line - 1, loc.start.column), new graphql_language_service_1.Position(loc.end.line - 1, loc.end.column));
            result.push({
                tag: '',
                template,
                range,
