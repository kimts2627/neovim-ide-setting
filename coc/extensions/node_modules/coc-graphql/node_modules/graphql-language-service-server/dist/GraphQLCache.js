...
async function getGraphQLCache({ parser, logger, loadConfigOptions, config, }) {
    ...
    if (!graphQLConfig) {
        graphQLConfig = await (0, graphql_config_1.loadConfig)(loadConfigOptions);
    }
...
class GraphQLCache {
    constructor({ configDir, config, parser, logger, }) {
        ...
        this.promiseToReadGraphQLFile = (filePath) => {
            return new Promise((resolve, reject) => fs.readFile(vscode_uri_1.URI.parse(filePath).fsPath, 'utf8', (error, content) => {
                if (error) {
                    reject(error);
                    return;
                }
                ...
                if (content.trim().length !== 0) {
                    try {
                        ...
                        queries.forEach(({ query }) => asts.push((0, graphql_1.parse)(query)));
                        resolve({
                            filePath,
                            content,
                            asts,
