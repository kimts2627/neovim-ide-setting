...
import { getAutocompleteSuggestions, getHoverInformation, validateQuery, getRange, DIAGNOSTIC_SEVERITY, getOutline, getDefinitionQueryResultForFragmentSpread, getDefinitionQueryResultForDefinitionNode, getDefinitionQueryResultForNamedType, getDefinitionQueryResultForField, getASTNodeAtPosition, getTokenAtPosition, getTypeInfo, } from 'graphql-language-service';
...
export class GraphQLLanguageService {
    ...
    async _getDefinitionForNamedType(query, ast, node, filePath, projectConfig) {
        ...
        const localOperationDefinitionInfos = typeCastedDefs.map((definition) => ({
            filePath,
            content: query,
            definition,
        }));
        const result = await getDefinitionQueryResultForNamedType(query, node, dependencies.concat(localOperationDefinitionInfos));
        return result;
    }
    async _getDefinitionForField(query, _ast, _node, _filePath, projectConfig, position) {
        ...
        if (typeInfo && fieldName) {
            ...
            const result = await getDefinitionQueryResultForField(fieldName, parentTypeName, dependencies);
            return result;
        }
        return null;
    }
